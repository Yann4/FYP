...
bool Collision::boundingBoxCollision(const AABB& a, const AABB& b, MTV& minTranslationVector)
{
	...
	/*std::array<DirectX::XMFLOAT3, 6> axes;
	axes[0] = rotateAxis(x, a.rotation);
	axes[1] = rotateAxis(y, a.rotation);
	axes[2] = rotateAxis(z, a.rotation);

	axes[3] = rotateAxis(x, b.rotation);
	axes[4] = rotateAxis(y, b.rotation);
	axes[5] = rotateAxis(z, b.rotation);

	if (!testAxis(axes[0], a.centre.x - a.size.x / 2, a.centre.x + a.size.x / 2, b.centre.x - b.size.x / 2, b.centre.x + b.size.x / 2, mtv))
	{
		return false;
	}

	if (!testAxis(axes[1], a.centre.y - a.size.y / 2, a.centre.y + a.size.y / 2, b.centre.y - b.size.y / 2, b.centre.y + b.size.y / 2, mtv))
	{
		return false;
	}

	if (!testAxis(axes[2], a.centre.z - a.size.z / 2, a.centre.z + a.size.z / 2, b.centre.z - b.size.z / 2, b.centre.z + b.size.z / 2, mtv))
	{
		return false;
	}


	if (!testAxis(axes[3], a.centre.x - a.size.x / 2, a.centre.x + a.size.x / 2, b.centre.x - b.size.x / 2, b.centre.x + b.size.x / 2, mtv))
	{
		return false;
	}

	if (!testAxis(axes[4], a.centre.y - a.size.y / 2, a.centre.y + a.size.y / 2, b.centre.y - b.size.y / 2, b.centre.y + b.size.y / 2, mtv))
	{
		return false;
	}

	if (!testAxis(axes[5], a.centre.z - a.size.z / 2, a.centre.z + a.size.z / 2, b.centre.z - b.size.z / 2, b.centre.z + b.size.z / 2, mtv))
	{
		return false;
	}*/
